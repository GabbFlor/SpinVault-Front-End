...
const Relacao_internacional = () => {
    ...
    const pegarDadosIniciaisDiscosInternacionais = async () => {
        setCarregando(true);

        try {
            const response = await axios.get(`${apiUrl}/discos/pegarVinteDiscosArtistasInternacionais/0`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })

            queryClient.setQueryData(["ultimoDocInternacionais"], response.data[response.data.length - 1]);

            const discosInternacionais = response.data;

            return discosInternacionais;
        } catch (error) {
            if (error.status === 403) {
                Swal.fire({
                    icon: "error",
                    title: "Erro!",
                    text: `A sua conta ainda não foi ativada para visualizar os discos internacionais! Assine um de nossos planos e tente novamente.`,
    ...
    const carregarProximaPagina = async () => {
        const ultimoDocCache = queryClient.getQueryData(['ultimoDocInternacionais']);
        ...
        try {
            const response = await axios.get(`${apiUrl}/discos/pegarVinteDiscosArtistasInternacionais/${ultimoDocCache.id}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            })
            ...
            if (discosInternacionais.length === 0) {
                alert("Não há mais nenhum conteúdo para ser carregado");
                setSumirBtn(true);
            } else {
                queryClient.setQueryData(['discosInternacionais'], (oldDiscos = []) => {
                    return [...oldDiscos, ...discosInternacionais];
                })
    ...
    const { data: discosInternacionais, isLoading, error } = useQuery({
        queryKey: ['discosInternacionais'],
        queryFn: pegarDadosIniciaisDiscosInternacionais,
    ...
    const handleUpdate = () => {
        queryClient.invalidateQueries(['discosInternacionais']);
        queryClient.invalidateQueries(['ultimoDocInternacionais']);
        setSumirBtn(false)
    }
                    ...
                    <tbody>
                        <tr>
                            <td colSpan="14" style={{ color: "red" }}>
                                {error.status === 403 ?
                                    ("Você não tem permissão para visualizar os discos internacionais.")
                                    :
        ...
        return (
            <div className="div-da-table">
                <table>
                    ...
                    <tbody>
                        {
                            discosInternacionais.length > 0 ? (
                                discosInternacionais.map((discoFiltrado) => (
                                    <tr key={discoFiltrado.id}>
                                        ...
                                        <td>
                                            {discoFiltrado.encarte == true ? "Sim" : "Não"}
                                        </td>
                                        <td>
        ...
        return (
            <div className="div-da-table">
                ...
                <table>
                    <thead>
                        <tr className="cell-title">
                            <th colSpan="5">Artistas internacionais</th>
                        </tr>
                        <tr className="cabecalho">
                            <th>Artista</th>
                            <th>Título</th>
...
export default Relacao_internacional;